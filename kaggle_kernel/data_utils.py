...
def find_data_root(local_dir: str = 'data', kaggle_root: str = '/kaggle/input') -> Path:
    """
    Find the data root directory either locally or in Kaggle environment.
    
    Parameters
    ----------
    local_dir : str
        Local directory name to check.
    kaggle_root : str
        Kaggle input root path.
    ...
    required = ['train.csv', 'test.csv', 'train_demographics.csv', 'test_demographics.csv']
...
def sample_balanced_split(
    ...
    ----------
    df : pd.DataFrame
        DataFrame with columns 'sequence_id', 'subject', 'gesture'.
    train_pct : float
        Percentage of total sequences to use for training.
    test_pct : float
        Percentage of total sequences to use for testing.
    random_state : int
