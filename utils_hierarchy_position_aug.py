...
# =============================================================================
# Advanced sequence extractor for hierarchy-position experiments
# - Bayes-safe multi-domain strings: "raw|velocity|displacement"
# - Optional simple Kalman / robust EKF-style smoothing
# - Optional dead-reckoning velocity/displacement features
# =============================================================================
...
class AdvancedMultiDomainSequenceExtractor(MultiDomainSequenceExtractor):
    ...
    def _as_modes(self, modes, single_mode):
        ...
        if isinstance(modes, str):
            if modes in ("", "none", "None"):
                return []
            if "|" in modes:
                return [m.strip() for m in modes.split("|") if m.strip() not in ("", "none", "None")]
            return [modes]
        return [m for m in list(modes) if m is not None and m != "none"]

    def _smooth_1d(self, values: np.ndarray, robust: bool = False) -> np.ndarray:
        ...
        state = x[0] if np.isfinite(x[0]) else 0.0
        p = 1.0
        out[0] = state
        for i in range(1, len(x)):
            z = x[i] if np.isfinite(x[i]) else state
            p = p + q
            k = p / (p + r)
            residual = z - state
            if robust:
                scale = np.sqrt(max(r, 1e-12)) * 3.0
                residual = scale * np.tanh(residual / scale)
            state = state + k * residual
            p = (1.0 - k) * p
            out[i] = state
        return out

    def _apply_motion_filter(self, X: pd.DataFrame) -> pd.DataFrame:
        if self.motion_filter_mode is None:
            return X.copy()
        if self.motion_filter_mode not in ("kalman", "extended_kalman"):
            raise ValueError(f"Unknown motion_filter_mode: {self.motion_filter_mode}")
        df = X.copy()
        robust = self.motion_filter_mode == "extended_kalman"
        sensor_cols = []
        sensor_cols += [c for c in ["acc_x", "acc_y", "acc_z", "rot_w", "rot_x", "rot_y", "rot_z"] if c in df.columns]
        sensor_cols += [c for c in df.columns if str(c).startswith("thm_")]
        sensor_cols += [c for c in df.columns if str(c).startswith("tof_")]
        sensor_cols = list(dict.fromkeys(sensor_cols))
        if not sensor_cols:
            return df
        for _, idx in df.groupby(self.sequence_col, sort=False).groups.items():
            for col in sensor_cols:
                df.loc[idx, col] = self._smooth_1d(df.loc[idx, col].to_numpy(dtype=float), robust=robust)
        if {"rot_w", "rot_x", "rot_y", "rot_z"}.issubset(df.columns):
            qcols = ["rot_w", "rot_x", "rot_y", "rot_z"]
    ...
    def _dead_reckoning_features(self, X: pd.DataFrame) -> pd.DataFrame:
        acc_cols = [c for c in ["acc_x", "acc_y", "acc_z"] if c in X.columns]
        if len(acc_cols) != 3:
            return pd.DataFrame(index=X.index)
        if self.counter_col in X.columns:
            ...
            dt = pd.Series(1.0 / float(self.sampling_rate), index=X.index)
        dt = dt.replace([np.inf, -np.inf], np.nan).fillna(1.0 / float(self.sampling_rate)).clip(lower=1e-6)
        acc = X[acc_cols].astype(float)
        vel = acc.mul(dt, axis=0).groupby(X[self.sequence_col].values, sort=False).cumsum()
        disp = vel.mul(dt, axis=0).groupby(X[self.sequence_col].values, sort=False).cumsum()
        out = pd.concat([vel, disp], axis=1)
        out.columns = ["acc_dr_vel_x", "acc_dr_vel_y", "acc_dr_vel_z", "acc_dr_disp_x", "acc_dr_disp_y", "acc_dr_disp_z"]
        return out

    def fit(self, X: pd.DataFrame, y: Optional[pd.DataFrame] = None) -> "AdvancedMultiDomainSequenceExtractor":
        X2 = self._apply_motion_filter(X)
        super().fit(X2, y)
        return self

    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        X2 = self._apply_motion_filter(X)
        out = super().transform(X2)
        if self.use_dead_reckoning:
            dr = self._dead_reckoning_features(X2)
            if not dr.empty:
                dr = dr.fillna(0.0)
                dr.index = X2[self.sequence_col].to_numpy()
                out = pd.concat([out, dr], axis=1)
        return out
